...
export default function StampEditor({ onComplete }: StampEditorProps) {
  ...
  // VALIDACE: Jsou vyplněny všechny fotky?
  ...
  useEffect(() => {
    const isPreview = mobileStep === activeTemplate.slots.length;
    if (isPreview) { setActiveSlotId(null); return; }
    const current = activeTemplate.slots[mobileStep];
    if (current) { setActiveSlotId(current.id); targetSlotIdRef.current = current.id; }
  }, [mobileStep, activeTemplate.slots]);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 2 && isDragging.current === 'photo' && initialTouchDistance.current && photos[activeSlotId]) {
      ...
      if (isDragging.current === 'photo' && photos[activeSlotId]) {
        ...
        const slotEl = document.querySelector(`[data-slot-id="${activeSlotId}"]`);
        if (slotEl) {
  ...
  const handleMobilePreview = async () => {
    setMobileStep(activeTemplate.slots.length);
  ...
  // KLÍČOVÝ FIX: Proměnná pro stín je zpět!
  ...
  return (
    <div 
      className="flex-1 min-h-0 w-full flex flex-col justify-between relative font-sans text-secondary select-none overflow-hidden touch-none"
      onMouseMove={(e) => {
        if (!isDragging.current || !activeSlotId) return;
    ...
    >
          ...
          <div className="relative shadow-2xl bg-black-custom border border-white/10 shrink-0" style={{ aspectRatio: `${activeTemplate.width} / ${activeTemplate.height}`, height: '100%', maxWidth: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            {activeTemplate.slots.map((slot) => (
              <div key={slot.id} data-slot-id={slot.id} className={`desktop-slot-${slot.id} absolute border border-dashed cursor-pointer overflow-hidden group transition-colors ${activeSlotId === slot.id ? 'border-primary bg-primary/10' : 'border-white/10 hover:border-white/30 bg-white/5'}`}
                ...
                {photos[slot.id] ? (
                  <div className="absolute inset-0 w-full h-full overflow-hidden" onMouseDown={(e) => { e.preventDefault(); handleMouseDown('photo', slot.id, e); }}>
                    ...
                    <div className="absolute bottom-0 right-0 w-6 h-6 bg-primary flex items-center justify-center cursor-se-resize z-20 shadow-md rounded-tl-md" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); handleMouseDown('resize', slot.id, e); }}>
                ...
                {slot.id === '1' && <div className="absolute cursor-move select-none p-2 whitespace-pre active:opacity-80 group w-max max-w-full touch-none"
                    ...
                    onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); handleMouseDown('text', '1', e); }}>
      ...
      <div className="flex md:hidden flex-col flex-1 min-h-0 w-full justify-between relative overflow-hidden">
        ...
        <div className="flex-1 min-h-0 w-full flex flex-col items-center justify-center p-6 bg-[#0F172A]">
          {isPreviewStep ? (
            ...
            <div data-slot-id={currentMobileSlot?.id} className={`mobile-slot-${currentMobileSlot?.id} relative shadow-2xl bg-white/5 border border-primary shrink-0`} style={{ aspectRatio: `${currentMobileSlot?.width} / ${currentMobileSlot?.height}`, height: '100%', maxWidth: '100%' }} onClick={() => handleSlotClick(currentMobileSlot!.id)}>
              {photos[currentMobileSlot!.id] ? (
                <div className="absolute inset-0 w-full h-full overflow-hidden" onTouchStart={(e) => handleTouchStart('photo', currentMobileSlot!.id, e)}>
                  ...
                  <button onClick={(e) => handleDeletePhoto(currentMobileSlot!.id, e)} className="absolute top-2 right-2 w-7 h-7 bg-red-500/90 text-white font-bold rounded-full text-xs flex items-center justify-center z-30 shadow-lg">✕</button>
                  <div className="absolute bottom-0 right-0 w-10 h-10 bg-primary flex items-center justify-center z-20 shadow-md" onTouchStart={(e) => { e.preventDefault(); e.stopPropagation(); handleTouchStart('resize', currentMobileSlot!.id, e); }}>
              ...
              ) : !isMobileTextStep && <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center p-4 text-center gap-2"><div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-lg">+</div><span className="text-xs font-semibold text-primary uppercase">Nahrát fotografii</span></div>}
              {isMobileTextStep && <div className="absolute cursor-move select-none p-2 whitespace-pre active:opacity-80 w-max max-w-full touch-none"
                    ...
                    onTouchStart={(e) => { e.preventDefault(); e.stopPropagation(); handleTouchStart('text', '1', e); }}>
      ...
      <footer className="sticky bottom-0 w-full bg-[#252C3C] border-t border-[#2B3755] h-[80px] md:h-[116px] flex items-center justify-center shrink-0 z-40">
        <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-[84px] flex justify-between items-center">
          <Button 
            onClick={() => {
              ...
              else { if (mobileStep > 0) setMobileStep(prev => prev - 1); else window.history.back(); }
            }} 
            variant="outlined" arrow="left"
          >Zpět</Button>
          <Button 
            onClick={() => {
              if (window.innerWidth >= 768) onComplete();
              else {
            ...
            arrow="right"
          >
            {window.innerWidth >= 768 ? 'Dokončit' : isPreviewStep ? 'Dokončit' : isLastSlotStep ? 'Náhled' : 'Další krok'}
          </Button>
        </div>
      </footer>
    </div>
