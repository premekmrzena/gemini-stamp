...
// Duplikát src/components/PurchaseCategoriesSection.tsx pro /partners - vlastní
// texty pro CK (content.ts) a bez next-intl (stránka je mimo [locale]).
// Vizuál drž v synchronu s originálem.
...
function CategoryImage({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="w-full relative min-w-0 min-h-0 aspect-[4/3] rounded-[4px] overflow-hidden">
      <Image src={src} alt={alt} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 25vw, 20vw" className="object-cover" />
    </div>
  );
}

export default function PartnerCategoriesSection({ lang }: { lang: PartnerLang }) {
  const t = CONTENT[lang].categories;
  return (
          ...
          return (
            ...
            >
              ...
              <div className="relative w-[80%] mb-4">
                <CategoryImage src={cat.image} alt={title} />
                {cat.featured && (
