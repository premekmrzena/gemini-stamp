...
| stock_quantity | integer, od 2026-07-22 se atomicky snižuje při vytvoření objednávky přes RPC `reserve_stock` (`docs/sql/015_products_reserve_stock.sql`), viz "Provedené SQL migrace" níže | ano | `0` |
...
| stock_released | boolean, od 2026-07-22 (`docs/sql/016_orders_stock_released.sql`) – idempotency flag pro `/api/stripe-webhook`: `true` znamená, že sklad rezervovaný při vytvoření objednávky byl vrácen kvůli neúspěšné platbě (a případně znovu zarezervován, pokud stejný PaymentIntent uspěl na retry) | ano | `false` |

RLS: `anon` smí `INSERT` (checkout) a `SELECT`, ale nemá žádnou `UPDATE` policy – objevilo se to 2026-07-22 při testování webhooku: `supabase.from('orders').update(...)` pod anon klíčem (`src/lib/supabase.ts`, používá ho i `/api/stripe-webhook`) neshodí chybu, jen potichu upraví 0 řádků (běžné PostgREST/RLS chování). Znamenalo to, že nastavení `status = 'Zaplaceno'` po úspěšné platbě fakticky nikdy nezapisovalo, jen o tom nikdo nevěděl, protože webhook nebyl v produkci zaregistrovaný ani reálně otestovaný (viz [sekce 1](01-technicka-infrastruktura.md#platby--stripe)). Opraveno `docs/sql/017_orders_webhook_rpc.sql` – dvě úzce zaměřené `SECURITY DEFINER` RPC (`mark_order_paid`, `set_order_stock_released`) místo širší RLS `UPDATE` policy pro anon, která by šla zneužít přes `orderId` v URL (`/dekujeme?orderId=...`) k přepsání libovolných polí cizí objednávky přímo přes Supabase REST API. `authenticated` (admin) má nadál plný CRUD beze změny.
