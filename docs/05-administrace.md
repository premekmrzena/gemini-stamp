...
Nad tabulkou je od 2026-07-26 i **filtr podle stavu** (select, „Všechny stavy“ + jednotlivé hodnoty z `ORDER_STATUSES`) vedle staršího datumového filtru — obojí se kombinuje (`filteredOrders`), promítá se i do souhrnných karet a ZIP exportu níže.

Vedle filtru je tlačítko **„Stáhnout tiskové archy (ZIP)“** (s info ikonkou vysvětlující chování na hover) — vezme všechny položky s `item_type === 'custom'` napříč aktuálně filtrovanými objednávkami (podle datumového i stavového filtru), stáhne jejich `print_url` z `custom_stamps` (dávkově, viz níže) a zabalí do jednoho ZIP (`jszip`, klientsky). Pojmenování souborů v ZIPu: `{posledních6znakůID objednávky}_{pořadí položky}.png`. Liší se od ikon u jednotlivého řádku (viz níže), které stahují jen arch z té jedné objednávky.
...
### Detail objednávky
- **Změna stavu** — vedle selectu se všemi 14 stavy (od 2026-07-26 seřazenými podle skutečného toku objednávky, ne náhodně) je tlačítko „Další krok“ s návrhem následujícího stavu v typické cestě objednávky (`getNextStatus`, dvě varianty podle `shipping_method`: osobní odběr → `Čekáme na platbu→Zaplaceno→Připravujeme→K vyzvednutí→Vyzvednuto→Uzavřeno`, doprava → `...→Odesláno→Doručeno→Uzavřeno`). Mimo tuto cestu (Zrušeno, Vráceno, Reklamace...) tlačítko nic nenabízí, jde jen přes select. Uložení je okamžité (`updateOrderStatus`), bez potvrzení a bez validace přechodu (lze přeskočit libovolně přes select)
- **Stav „Čekáme na platbu“ + badge „Nová“** (od 2026-07-26, viz [sekce 2](02-stavy-objednavky.md)) — objednávka se teď vytváří rovnou se stavem `Čekáme na platbu` (dřív `Nová` u obou platebních metod, což u bankovního převodu matlo „právě vytvořeno“ s „pořád čeká na peníze“). `Nová` je teď jen dočasný badge (`isFreshOrder`, 24 hodin od vytvoření) zobrazený vedle skutečného stavu — v seznamu objednávek, mobilní kartě i v hlavičce detailu. Řádky se stavem `Čekáme na platbu`/`Zaplaceno` mají navíc jemně odlišné podbarvení (`getOrderRowBg`) pro rychlou vizuální orientaci.
- **Historie stavů** (od 2026-07-26) — svislá časová osa se všemi dosavadními stavy objednávky a přesným časem změny. Plní se DB triggerem na `orders` (`docs/sql/024_order_status_history.sql`, tabulka `order_status_history`) — loguje se každá změna `status` bez ohledu na to, jestli ji vyvolá dashboard, Stripe webhook (`mark_order_paid`) nebo cokoli budoucího, takže časová osa nemůže "zapomenout" na změnu udělanou mimo dashboard. Objednávky vytvořené před spuštěním migrace nemají zpětnou historii.
