...
- **Živý provoz od 2026-08-06.** Jeden Stripe účet – EUR je nastavené jako default settlement currency, CZK jako druhá vyplácecí měna se samostatným bankovním účtem (Air Bank) napojeným na ten samý Stripe účet (Stripe Dashboard → Settings → Bank accounts and currencies). Obě měny se tak vyplácí bez konverze/Stripe fee, žádné dva oddělené účty/API klíče nejsou potřeba (krátce se zvažovalo, viz `docs/00-platby-meny-konverze.md`). `src/lib/stripe.ts` (`getStripeClient()`) staví Stripe klienta líně, ať chybějící env proměnná při buildu nespadne celou appku. Payout schedule: Automatic/Weekly (pondělí), Accelerated payout speed (3 pracovní dny), statement descriptor `My Creative Stamp`.
- Flow:
  ...
  2. `POST /api/create-payment-intent` – server načte `total_price`/`currency` objednávky z DB (opět nedůvěřuje klientovi) a vytvoří Stripe PaymentIntent v dané měně (`payment_method_types: ['card']`)
...
- **Live webhook zaregistrovaný a živě otestovaný** (2026-08-06) – endpoint `https://mycreativestamp.com/api/stripe-webhook`, eventy `payment_intent.succeeded`, `payment_intent.payment_failed`, `payment_intent.canceled`, `STRIPE_WEBHOOK_SECRET` doplněný v `.env.local` i Vercel Production. `/api/stripe-webhook` je záměrně vyjmutý z pre-launch gate (viz [sekce 8](08-pre-launch-gate.md)).
- **Reálný end-to-end test proveden 2026-08-06**: objednávka 30 Kč (CZK) a 1 € (EUR), obě `Zaplaceno`, iDoklad faktura vystavena a e-mailem doručena u obou (číselná řada `20260001`/`20260002` – testovací faktury z předchozích kol byly smazané, viz níže). Výplata do banky (CZK i EUR, bez konverze) čeká na potvrzení – Accelerated payout speed = 3 pracovní dny od platby, pak nejbližší týdenní cyklus.
- **Pozor na minimální částku u CZK plateb** – Stripe má pro CZK charge minimum kolem 15 Kč; testovací produkt proto musí mít cenu nad touhle hranicí (použito 30 Kč), jinak `create-payment-intent` selže už při vytváření PaymentIntentu.
- **Vymazání testovacích dat před live testem** (2026-08-06) – smazáno: všechny řádky `orders`/`custom_stamps`/`discount_codes`, 2 testovací Blob soubory (`editor-orders/...`), 2 vystavené iDoklad faktury (`DELETE /v3/IssuedInvoices/{id}` – účet je neplátce DPH, mazání testovacích dokladů před ostrým provozem je vědomě opakovaný postup, stejně jako 2026-07-26/27), `sold_count` vynulován a `stock_quantity` sjednocen na 100 ks u všech 45 produktů (starší testovací nákupy nechaly u pár produktů záporný/nekonzistentní sklad, přesné dopočítání z dřívějších smazaných objednávek nebylo možné).
...
- **Stripe výplata do banky – čeká na potvrzení** (viz sekce Platby výše) – ověřit za pár dní, že CZK i EUR opravdu přistály na správný účet bez konverze, pak rozhodnout, zda zůstatek z testovacích plateb (30 Kč + 1 €) refundovat nebo nechat.
...
## Změny
- 2026-08-09: **Tiskový arch dočasně obsahuje šablonu na pozadí** – po dobu, kdy se fotky i šablona archu tisknou zároveň na prázdný papír (ne dotisk na předtištěné archy), generuje editor `custom_stamps.print_url` v plném rozlišení se šablonou na pozadí, stejně jako náhled. Řízeno jedním přepínačem `PRINT_INCLUDES_TEMPLATE_BACKGROUND` v `src/lib/editorConfig.ts` (výchozí `true`); `generateCanvasDataUrl()` v `src/lib/canvasUtils.ts` uměl obě varianty už dřív (parametr `includeBackground`), jen se pro tisk dosud nepoužívala. Až se přejde zpět na dotisk na předtištěné archy, stačí flag vrátit na `false` – viz [sekce 4](04-popis-eshopu.md#3-editor-kreativní-archy).
- 2026-08-09b: **Samostatný tiskový podklad šablony bez ilustrace známek** – protože se skutečné známky nalepují na vytištěný arch ručně, tiskový podklad (na rozdíl od editorového `*_white-slots.jpg`) nesmí obsahovat jejich ilustraci, jen prázdné místo se slabým rámem. `getPrintBackgroundImage()` v `editorConfig.ts` odvozuje cestu k novému souboru `..._print-slots.jpg` ze stávajícího `backgroundImage` (1:1 rozměr/sloty, jen jiná přípona). Soubory pro 5 šablon zatím nejsou dodané – `generateCanvasDataUrl()` proto při chybějícím/nenačitatelném podkladu tiše spadne zpět na `*_white-slots.jpg`, aby se objednávky nezasekly; jakmile se soubory nahrají do `/public/templates/` pod správným názvem, začnou se používat samy bez nasazení kódu.
- 2026-08-06: **Stripe live spuštění dokončeno** – live klíče + webhook (viz sekce Platby výše), reálný end-to-end test v CZK i EUR úspěšný. Krátce se zvažovalo rozdělení na dva samostatné Stripe účty (CZK/EUR) - zjištěno, že to není potřeba: jeden živý účet má EUR jako default settlement currency a CZK jako druhou vyplácecí měnu (vlastní bankovní účet napojený na stejný Stripe login), obojí bez konverze. `src/lib/stripe.ts` (líný `getStripeClient()`/`getStripeWebhookSecret()`) zůstává jen kvůli bezpečné líné inicializaci, ne kvůli routingu mezi účty. Před testem vymazána všechna testovací data (objednávky, iDoklad faktury, Blob, sklad sjednocen na 100 ks u všech produktů). Souvisí s [sekcí 0](00-platby-meny-konverze.md).
