...
- **hreflang (`alternates.languages`)** – zatím nikde nenastaveno. Vědomě odloženo: jediná reálná produkční mutace je `en` (`cs` je jen interní náhled, needitovatelný/needindexovatelný pro boty, ko/ja/zh-* mají prázdné `messages/*.json`, viz [sekce 9](09-jazykove-mutace.md)). Přidat až bude aspoň druhá reálná zákaznická mutace.
