...
- **`src/app/robots.ts`** – povoluje `/`, disallow `/admin`, `/api`, `/kosik`, `/dekujeme`, `/ko`, `/ja`, `/zh-Hans`, `/zh-Hant`. Odkazuje na `${SITE_URL}/sitemap.xml`. `/cs/...` se do robots.txt záměrně nedává – je řešené na úrovni `proxy.ts` (redirect pryč pro kohokoli bez `site_access` cookie, viz výše), takže ho crawler nikdy neuvidí ani nemá šanci zaindexovat.
- **`/ko`, `/ja`, `/zh-Hans`, `/zh-Hant` disallow (2026-08-20):** tyhle 4 mutace jsou v `routing.ts` připravené dopředu, ale `messages/*.json` jsou zatím prázdné – prefixovaná URL přesto next-intl normálně routuje (fallback na anglický text), `canonical` na nich správně míří na neprefixovanou `/en` cestu. Google je procházel a hlásil v GSC jako „Alternativní stránka se správnou značkou kanonické stránky" (7 stránek) – nešlo o chybu (canonical fungoval správně, nic se neindexovalo duplicitně), ale disallow to zastaví u zdroje, ať se nafukování hlášení neopakuje. Až některá z mutací dostane reálný obsah, je potřeba ji z tohohle disallow pole odebrat.
...
- 2026-08-20: GSC nahlásil nový důvod vyloučení „Alternativní stránka se správnou značkou kanonické stránky" (7 stránek, vše `/zh-Hans/...` a `/zh-Hant/...`) – diagnostikováno jako benigní (canonical správně míří na neprefixovanou `/en` cestu, nešlo o chybu), ale `/ko`, `/ja`, `/zh-Hans`, `/zh-Hant` přidány do `robots.ts` disallow, ať Google tyhle zatím prázdné mutace vůbec neprochází a hlášení se nenafukuje.
