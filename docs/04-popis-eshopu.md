...
> Obecné shrnutí toho, co e-shop dělá a jak se v něm zákazník pohybuje, bez nutnosti číst kód. Popis odpovídá skutečnému chování k 2026-06-16.
...
Pod Hero sekcí je výpis produktů. Zobrazují se jen produkty, které mají `is_active = true` **a** `show_on_homepage = true`, řazené podle `tag_top` (produkty s nastaveným TOP pořadím první) a pak podle data vytvoření (nejnovější první).

Každý produkt může mít libovolnou kombinaci těchto štítků současně (zobrazí se vedle sebe, nejde o výběr jen jednoho):
...
Kliknutím na produkt se zákazník dostane na detail (`/produkt/[id]`): hlavní obrázek + galerie, název, cena, technické parametry (katalogové číslo, typ známky, datum vydání, rozměry, designér, rytec…) a popis v rozbalovacích sekcích, plus 3 související produkty na konci stránky. Tlačítkem „Do košíku“ se produkt přidá do košíku přímo z detailu i z karty produktu na výpisu.
...
Po odeslání formuláře se zavolá `/api/create-order`, který znovu ověří ceny i dostupnost položek proti databázi (nikdy nevěří cenám z prohlížeče) a vytvoří objednávku se stavem `Nová`. Dál se liší podle zvolené platby:
