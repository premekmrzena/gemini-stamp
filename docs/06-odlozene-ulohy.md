...
| **CN22 celní nálepka pro `VL`/`EM` — nejasné, jestli je potřeba (KRITICKÉ pro plán plateb v CNY/JPY/KRW/TWD, viz [sekce 0](00-platby-meny-konverze.md))** | Prozkoumáno 2026-08-05 (viz [sekce 10](10-doprava-a-celni-prohlaseni.md#technické-detaily-napi-b2b-zsk)) — samostatné endpointy `/letterWithCN22*` existují, ale odmítají odkaz na naši existující zásilku (chtějí kompletní novou adresu, `INVALID_PREFIX` na náš `VL` kód). Možné, že naše stávající elektronické celní prohlášení (`parcelCustomsDeclaration` v `/parcelService`) už stačí, ale to je pořád jen "možné" - needs-confirm od ČP. Navíc jednotlivé cílové země mohou mít vlastní dodatečný požadavek nad rámec ČP (analogie k Zonos Declaration ID u USA) - např. Jižní Korea vyžaduje u drtivé většiny osobních zásilek příjemcův **PCCC (Personal Customs Clearance Code)** uvedený v celní deklaraci, jinak zásilka uvázne na korejské celnici. Nemáme zatím ověřeno, jestli podobný požadavek platí i pro Čínu/Japonsko/Taiwan. | Dotaz na CN22/elektronické celní prohlášení odeslán ČP 2026-08-05, čeká se na odpověď - needs-confirm, neimplementovat naslepo (celní/compliance oblast). Než se otevře prodej do CN/JP/KR/TW (viz plán v [sekci 0](00-platby-meny-konverze.md)), navíc ověřit u ČP/celních zdrojů zemi po zemi, jestli existuje obdoba PCCC nebo jiný dodatečný požadavek - jinak hrozí, že zaplacená zásilka nedorazí. |
